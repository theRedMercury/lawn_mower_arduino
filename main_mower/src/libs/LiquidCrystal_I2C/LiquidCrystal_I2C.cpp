...
inline size_t LiquidCrystal_I2C::write(unsigned char value)
{
	send(value, Rs);
	return 1;
}
...
inline void LiquidCrystal_I2C::write(unsigned char value)
{
	send(value, Rs);
}
...
LiquidCrystal_I2C::LiquidCrystal_I2C(unsigned char lcd_Addr, unsigned char lcd_cols, unsigned char lcd_rows)
{
	_Addr = lcd_Addr;
	_cols = lcd_cols;
	_rows = lcd_rows;
